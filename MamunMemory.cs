...
namespace BLACKSKYMEM
{
    public class BLACKSKYSAFEMEM
    {
        ...
        {
            if (_processHandle != IntPtr.Zero && processId > 0)
            {
                try
                {
        ...
        private async Task<IEnumerable<long>> AobScan(string pattern)
        {
            ...
            ConcurrentBag<long> addressRet = new ConcurrentBag<long>();
            ...
            await Task.Run(() =>
            {
                ...
                var options = new ParallelOptions { MaxDegreeOfParallelism = Math.Max(2, maxThreads - 2) };

                Parallel.ForEach(list, options, addresss =>
                {
                    int regionSize = addresss.Size;
                    byte[] buffer = new byte[regionSize]; // Create managed byte array

                    if (ReadProcessMemory(_processHandle, addresss.Start, buffer, (IntPtr)regionSize, out IntPtr bytesRead))
                    {
                        int num = -patternLength;
                        do
                        {
                            num = FindPattern(buffer, patternData.pattern, patternData.mask, num + patternLength);
                            if (num >= 0)
                            {
        ...
        {
            return page.State == 0x1000 && page.Type == 0x20000 && page.Protect == 0x04;
        }
        ...
        public byte[] ReadMemory(long addressToRead, int size)
        {
            byte[] buffer = new byte[size];
            if (ReadProcessMemory(_processHandle, (IntPtr)addressToRead, buffer, (IntPtr)size, out var _))
            {
                return buffer;
            }
            return new byte[size]; // Return empty buffer if read fails
        }
        ...
        internal bool WriteMemory(byte[] searchBytes, byte[] replaceBytes)
        {
