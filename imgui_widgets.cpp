...
static std::unordered_map<ImGuiID, ImVec4> g_text_anim_color;

static ImVec4 _AnimColorStep(ImGuiID id, const ImVec4& target)
{
	ImVec4 cur = target;
	auto it = g_text_anim_color.find(id);
	if (it != g_text_anim_color.end()) cur = it->second;
	float dt = ImGui::GetIO().DeltaTime;
	float k = 1.f - powf(0.5f, dt * 12.f);
	k = ImClamp(k, 0.f, 1.f);
	ImVec4 out = ImLerp(cur, target, k);
	if (fabsf(out.x - target.x) < 0.001f &&
		fabsf(out.y - target.y) < 0.001f &&
		fabsf(out.z - target.z) < 0.001f &&
		fabsf(out.w - target.w) < 0.001f) out = target;
	g_text_anim_color[id] = out;
	return out;
}



void ImGui::TextColoredAnim(const char* key, const ImVec4& col, const char* fmt, ...)
{
	va_list args;
	va_start(args, fmt);
	ImGuiID id = ImGui::GetID(key);
	ImVec4 smooth = _AnimColorStep(id, col);
	PushStyleColor(ImGuiCol_Text, smooth);
	TextV(fmt, args);
	PopStyleColor();
	va_end(args);
}
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	window->DrawList->AddText(bb.GetCenter() - label_size / 2, c::label::active, label);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
	...
	rainbow.RenderRect(grab_rect.Min, grab_rect.Max, c::elements::rounding, true, 1.f, "scroll", 1.f);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	...
	if (!is_multiline)
	{
		GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(c::child::background), c::elements::rounding);
		GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(c::child::background2), c::elements::rounding);
	}
	...
	if (render_cursor || render_selection)
	{
		...
		if (render_cursor)
		{
			...
			if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect)) draw_window->DrawList->AddLine(ImVec2(total_bb.Min.x + state_anim.slow, cursor_screen_rect.Min.y), ImVec2(total_bb.Min.x + state_anim.slow, cursor_screen_rect.GetBL().y), GetColorU32(c::text::text_active));
